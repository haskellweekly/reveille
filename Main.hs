...
getPort = do
  ...
  pure . Maybe.fromMaybe 8080 . Read.readMaybe $ Maybe.fromMaybe
    ""
    maybeRawPort
...
logger request status _ = putStrLn $ concat
...
fromUtf8 bytes = case Text.decodeUtf8' bytes of
...
fetchFeed feed = do
  ...
  Conduit.runConduitRes
    ...
    $ \exception -> pure . Left $ Exception.displayException
        (exception :: Exception.SomeException)

withHeaders :: Client.Request -> Client.Request
withHeaders request =
  Client.addRequestHeader Http.hAccept "*/*"
    $ Client.addRequestHeader Http.hUserAgent (toUtf8 "reveille") request
...
parseFeed
  ...
  -> Conduit.ConduitM
       ...
       (Either String [Either String Item])
parseFeed format = case format of
...
extractAtomItem atomFeed atomEntry = Right Item
...
fromAtomText atomText = case atomText of
  Atom.AtomPlainText textType text -> case textType of
    Atom.TypeText -> text
...
extractRssItem rssDocument rssItem = do
  itemTime <- case Rss.itemPubDate rssItem of
    Nothing -> Left "no pub date"
    Just time -> Right time
  itemLink <- case Rss.itemLink rssItem of
...
runServer settings = Warp.runSettings settings . applicationWith
...
getRecentItems connection = Sqlite.query_
  connection
...
itemsToHtml items = Lucid.doctypehtml_ $ mconcat
  [ Lucid.head_ $ mconcat
...
itemToHtml item = Lucid.li_ $ mconcat
...
itemsToAtom time items = do
  ...
  Right Atom.AtomFeed
    ...
    , Atom.feedUpdated = Maybe.fromMaybe time
      . Maybe.listToMaybe
...
toAtomPerson name = do
  personName <- maybe (Left "toAtomPerson: empty name") Right
    $ NonNull.fromNullable name
  Right Atom.AtomPerson
...
renderAtom atom = Conduit.connect
  (Conduit.fuse
